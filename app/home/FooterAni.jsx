...
const FooterAni = () => {
  ...
  // Section/column refs for premium reveal effect
  const leftColRef = useRef(null);
  const quickLinksRef = useRef(null);
  const findUsRef = useRef(null);
  const boringColRef = useRef(null);

  // Buttons for hover animation
  const socialBtnRefs = useRef([]);
  ...
  useEffect(() => {
    ...
    const columns = [
      ...
      findUsRef.current,
      boringColRef.current,
    ].filter(Boolean);
    if (columns.length > 0) {
      gsap.fromTo(
        columns,
        { opacity: 0, y: 60, scale: 0.96, rotateX: 30 },
        {
          opacity: 1,
          y: 0,
          scale: 1,
  ...
  // Social button hover animation (premium GSAP touch)
  useEffect(() => {
    const buttons = socialBtnRefs.current.filter(Boolean);
    const cleanupFns = [];
    ...
    buttons.forEach((btn, i) => {
      ...
      const hoverIn = () => {
        gsap.to(btn, {
          scale: 1.125,
          rotate: i % 2 === 0 ? 6 : -6, // wiggle premium touch
          boxShadow: "0 4px 28px 0 rgba(196,23,24,0.17)",
      ...
      // Save cleanup function for each btn
      cleanupFns.push(() => {
  ...
  return (
    <footer
      ref={footerRef}
      className="w-full bg-black pt-[3vw] pb-[1vw] mt-[3vw] overflow-clip"
    // ensure content not outside on anim
    >
      <div className="max-w-[100vw] mx-auto px-[.5vw] gap-[2vw] flex flex-col md:flex-row justify-between items-start text-white border-b border-[#262626] pb-[5vw]">
        ...
        <div
          ref={leftColRef}
          className="flex flex-col items-start md:w-1/4 mb-[2vw] md:mb-0"
        >
          ...
          <div className="flex space-x-3 w-full mt-1 px-[1vw]">
            {[
              ...
              <button
                key={data.label}
                ref={el => (socialBtnRefs.current[idx] = el)}
                className="w-11 h-11 bg-[#161616] flex items-center justify-center rounded transition-all duration-200"
                aria-label={data.label}
                tabIndex={0}
                type="button"
              // no onMouseEnter here, GSAP will handle event
              >
                {data.svg}
              </button>
            ))}
        ...
        <div
          ref={quickLinksRef}
          className="flex flex-col md:w-1/4 mt-[1.6vw] md:mb-0"
        >
          <h3
        ...
        <div
          ref={findUsRef}
          className="flex flex-col md:w-1/4 mb-[2vw] mt-[1.3vw] md:mb-0"
        >
          <h3
            className="font-bold text-white text-[1.2vw] uppercase tracking-wide mb-[1vw] leading-none"
            style={{ letterSpacing: "0.06em" }}
